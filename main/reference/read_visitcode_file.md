...
- ...:
  ...
  All other columns from the input file (including optional CYCDAY)
...
When present, each treatment cycle D1 row may specify the interval in
days from the previous cycle D1. Used by
...
when `cycle_days` is `NULL`. Non-D1 rows and C1D1 may leave CYCDAY
empty.
