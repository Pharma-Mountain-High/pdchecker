...
- eot_dataset:
  ...
  Character vector, end of treatment dataset names (default: "EOT").
  Multiple datasets can be specified; the latest EOT date per subject is
  used. If no dataset exists, EOT-related planned dates will be NA

- eot_date_var:

  Character vector, end of treatment date variable names (default:
  "EOTDAT").
...
- cycle_days:
  ...
  Numeric or NULL. Treatment cycle length in days. When set, all cycles
  use this same interval. When `NULL` (default), intervals are read from
  the `CYCDAY` column on each treatment D1 row in `visitcode` (days from
  the previous cycle D1).
