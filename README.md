...
| 列名     | 说明                                       | 示例   |
| ------ | ---------------------------------------- | ---- |
| CYCDAY | 治疗周期 D1 与上一周期 D1 的间隔天数；仅 D1 行填写，供 `generate_planned_visit_dates()` 在 `cycle_days = NULL` 时使用 | 21   |

**CYCDAY 与 `cycle_days` 参数：**

- 不传 `cycle_days`（默认 `NULL`）：从 visitcode 的 **CYCDAY** 列按周期读取间隔（C2、C3… 各自的 D1 行）
- 传入 `cycle_days = 28` 等正数：**优先使用参数**，所有周期间隔统一为该值，忽略 CYCDAY
