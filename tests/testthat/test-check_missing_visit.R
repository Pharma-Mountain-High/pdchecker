...
test_that("check_missing_visit 肿瘤评估访视：规则与治疗期一致", {
  planned_dates <- data.frame(
    SUBJID = "001",
    VISIT = c("W8", "W16"),
    VISITNUM = c(8, 16),
    visittype = rep("肿瘤评估", 2),
    visitday = c("56", "112"),
    visit_category = rep("tumor_assessment", 2),
    planned_date = as.Date(c("2024-02-26", "2024-04-22")),
    wp_start = as.Date(NA),
    wp_end = as.Date(NA),
    wp_type = NA_character_,
    wp_value = NA_real_,
    actual_date = as.Date(c("2024-02-26", NA)),
    status = c("completed", "missing"),
    first_dose_date = as.Date("2024-01-01"),
    eot_date = as.Date("2024-06-01"),
    eos_date = as.Date("2024-08-01"),
    stringsAsFactors = FALSE
  )
  ...
  expect_true(result$has_deviation)
  expect_equal(nrow(result$details), 1)
  expect_equal(result$details$VISIT[1], "W16")
  expect_equal(result$details$visittype[1], "肿瘤评估")
})

test_that("check_missing_visit 肿瘤评估访视：计划日期未到达截止时不检查", {
  planned_dates <- data.frame(
    SUBJID = "001",
    VISIT = "W16",
    VISITNUM = 16,
    visittype = "肿瘤评估",
    visitday = "112",
    visit_category = "tumor_assessment",
    planned_date = as.Date("2024-04-22"),
    wp_start = as.Date(NA),
    wp_end = as.Date(NA),
    wp_type = NA_character_,
    wp_value = NA_real_,
  ...
  result <- check_missing_visit(planned_dates, cutoffdt = as.Date("2024-04-01"))
