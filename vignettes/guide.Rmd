...
从治疗结束数据集中提取每位受试者的 EOT 日期。支持多个 EOT 数据集，取每位受试者的**最大**日期。
...
| `eot_dataset` | 字符向量 | `"EOT"` | EOT 数据集名称，可多个；取最大 EOT 日期 |
| `eot_date_var` | 字符向量 | `"EOTDAT"` | EOT 日期变量 |
| `ds_dataset` | 字符串 | `"DS"` | DS 数据集名称 |
...
本手册基于 pdchecker 包当前版本（0.9.1）编写。如有更新，请参阅包的 `NEWS.md` 文件。
