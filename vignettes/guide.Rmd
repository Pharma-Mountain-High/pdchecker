...
- **报告生成**：将检查结果导出为 Excel、HTML 或 Markdown 格式报告
...
- [2. 准备工作](#2-准备工作)
  - [2.1 read_raw_data() — 读取 SAS 数据](#21-read_raw_data--读取-sas-数据)
  - [2.2 read_raw_data_with_formats() — 使用 SAS catalog 读取](#22-read_raw_data_with_formats--使用-sas-catalog-读取)
  - [2.3 read_visitcode_file() — 读取访视计划文件](#23-read_visitcode_file--读取访视计划文件)
  - [2.4 read_testconfig_file() — 读取检查配置文件](#24-read_testconfig_file--读取检查配置文件)
  - [2.5 set_pdchecker_options() / get_pdchecker_options() — 全局选项配置](#25-set_pdchecker_options--get_pdchecker_options--全局选项配置)
- [3. 日期提取函数](#3-日期提取函数)
  - [3.1 get_first_dose_date() — 首次给药日期](#31-get_first_dose_date--首次给药日期)
  - [3.2 get_last_dose_date() — 末次给药日期](#32-get_last_dose_date--末次给药日期)
  - [3.3 get_eot_date() — 治疗结束日期](#33-get_eot_date--治疗结束日期)
  - [3.4 get_eos_date() — 研究结束日期](#34-get_eos_date--研究结束日期)
- [4. 数据准备函数](#4-数据准备函数)
  - [4.1 generate_planned_visit_dates() — 生成计划访视日期](#41-generate_planned_visit_dates--生成计划访视日期)
  - [4.2 prepare_test_data() — 准备检查项数据](#42-prepare_test_data--准备检查项数据)
- [5. 知情同意类检查](#5-知情同意类检查)
  - [5.1 check_icf_time_deviation() — ICF 签署前操作](#51-check_icf_time_deviation--icf-签署前操作)
  - [5.2 check_screen_without_ic() — 未签署知情同意](#52-check_screen_without_ic--未签署知情同意)
- [6. 访视合规类检查](#6-访视合规类检查)
  - [6.1 check_missing_visit() — 遗漏访视](#61-check_missing_visit--遗漏访视)
  - [6.2 check_visit_window() — 访视超窗](#62-check_visit_window--访视超窗)
- [7. 检查项缺失类检查](#7-检查项缺失类检查)
  - [7.1 check_missing_test() — 检查项缺失](#71-check_missing_test--检查项缺失)
- [8. 结果汇总与报告生成](#8-结果汇总与报告生成)
  - [8.1 combine_check_results() — 合并检查结果](#81-combine_check_results--合并检查结果)
  - [8.2 capture_check_results() — 批量执行检查](#82-capture_check_results--批量执行检查)
  - [8.3 generate_excel_report() — Excel 报告](#83-generate_excel_report--excel-报告)
  - [8.4 generate_html_report() — HTML 报告](#84-generate_html_report--html-报告)
  - [8.5 generate_markdown_report() — Markdown 报告](#85-generate_markdown_report--markdown-报告)
- [9. 工具函数](#9-工具函数)
  - [9.1 is_sas_na() — SAS 缺失值判断](#91-is_sas_na--sas-缺失值判断)
  - [9.2 as_check_df() — 检查结果转数据框](#92-as_check_df--检查结果转数据框)
  - [9.3 apply_format_mapping() — 格式映射](#93-apply_format_mapping--格式映射)
- [附录](#附录)
  - [A.1 全局选项完整列表](#a1-全局选项完整列表)
  - [A.2 返回值字段说明](#a2-返回值字段说明)
  - [A.3 常见问题](#a3-常见问题)
...
### 2.2 read_raw_data_with_formats() — 使用 SAS catalog 读取

使用 SAS 格式目录（`.sas7bcat`）读取数据并进行编码映射，适用于数据中包含编码值需要转换的场景。

```r
data <- read_raw_data_with_formats(
  data_dir     = "path/to/sas/files",
  catalog_file = "path/to/formats.sas7bcat",
  iwrs_file    = "path/to/iwrs.csv",   # 可选
  encoding     = "GBK"                 # 中文数据常用 "GBK"
)
```
...
**返回值：** tibble，包含原文件所有列，加上自动生成的 `type`、`wpvalue`、`visit_category`（若有 CYCLE 列）。
...
### 2.4 read_testconfig_file() — 读取检查配置文件
...
set_pdchecker_options(
  ...
  sv_date_var     = "SVSTDTC",  # 访视日期变量（项目中可能不同）
  ex_datasets     = c("EC"),    # 用药数据集（如 EC 而非 EX）
  ex_date_var     = "ECSTDTC",  # 用药日期变量
  ic_dataset      = "IC",       # 知情同意数据集
  ic_date_var     = "ICDTC",    # 知情同意日期变量
  test_date_var   = "LBDTC",    # 检查日期变量
  test_cat_var    = "LBCAT"     # 检查类别变量
)
...
**优先级**：函数调用时直接传参 > 全局选项 > 硬编码默认值。即使设置了全局选项，仍可在个别调用中传参覆盖。
...
# 使用结束日期
last_dates <- get_last_dose_date(data, ex_end_date_var = "EXENDAT")
```
...
从数据列表中提取指定的检查数据集，与访视数据合并，生成标准化的数据框供 `check_missing_test()` 使用。
...
**返回值：** 标准化的数据框，列名统一为 `SUBJID`、`VISIT`、`VISITNUM`、`TBNAME`、`TESTCAT`、`TESTDE`、`TESTYN`、`TESTDAT`、`ORRES` 等。
...
> **注意**：`TESTYN` 字段使用中文 **"是"** 表示已执行，与数据录入规范一致。
...
### 8.2 capture_check_results() — 批量执行检查

将多个检查包装成函数后批量执行，自动捕获输出并合并结果。每个参数必须是一个接受 `data` 参数的**函数**。

**参数：**

| 参数 | 类型 | 说明 |
|------|------|------|
| `...` | 命名函数 | 每个参数是一个 `function(data)` 形式的函数 |
| `data` | 任意 | 传递给每个函数的数据参数 |

```r
all_results <- capture_check_results(
  icf_check = function(data) check_icf_time_deviation(data),
  ic_check  = function(data) check_screen_without_ic(data),
  mv_check  = function(data) check_missing_visit(planned_dates),
  vw_check  = function(data) check_visit_window(planned_dates),
  mt_check  = function(data) check_missing_test(prepared_lb),
  data = data
)
```

> **注意**：参数值必须是函数（`function(data) ...`），不能直接传入检查调用的结果。

---

### 8.3 generate_excel_report() — Excel 报告

生成包含 Summary（汇总统计）和 All Deviations（逐条明细）两个工作表的 Excel 文件。
...
| `include_no_deviation` | 逻辑值 | `FALSE` | 是否包含无偏离的检查项 |
...
### 8.4 generate_html_report() — HTML 报告

将结果渲染为自包含的 HTML 文件，可指定自定义 CSS。
...
| `include_no_deviation` | 逻辑值 | `FALSE` | 是否包含无偏离的检查项 |
| `title` | 字符串 | `"Study Deviation Report"` | 报告标题 |
| `css_file` | 字符串 | `NULL` | 自定义 CSS 文件路径 |
...
### 8.5 generate_markdown_report() — Markdown 报告
...
|------|------|--------|------|
| `checks_df` | 数据框 | （必填） | 合并后的检查结果 |
| `output_file` | 字符串 | `NULL` | 输出文件路径（为 NULL 时仅返回内容不写文件） |
| `include_no_deviation` | 逻辑值 | `FALSE` | 是否包含无偏离的检查项 |
| `title` | 字符串 | `"Study Deviation Report"` | 报告标题 |
...
generate_markdown_report(
  checks_df   = all_results,
  output_file = "deviation_report.md",
  title       = "方案偏离检查报告"
)
```
...
### 9.3 apply_format_mapping() — 格式映射

将格式映射应用到数据集，类似 SAS 的 format 功能。
...
|------|------|------|
| `data` | 数据框 | 要映射的数据 |
| `format_df` | 数据框 | 包含 `FMTNAME`、`START`、`LABEL` 列的格式映射表 |
...
result <- apply_format_mapping(data$DM, format_df)
```
...
**Q4: visitcode 和 testconfig 变量名必须固定吗？**
...
**Q5: 如何避免每个函数都重复指定相同参数？**

使用 `set_pdchecker_options()` 在脚本开头统一设置，详见 [2.5 全局选项配置](#25-set_pdchecker_options--get_pdchecker_options--全局选项配置)。

**Q6: check_missing_test 中 TESTYN 用什么值表示已执行？**

使用中文 **"是"**。若数据中使用 "Y" 等格式，需在调用 `prepare_test_data()` 前进行数据转换。

**Q7: 如何忽略特定日期变量的 ICF 检查？**
...
本手册基于 pdchecker 包当前开发版本（0.0.0.9000）编写。如有更新，请参阅包的 `NEWS.md` 文件。
